...
static uint8_t Get_Chip_ID(uint8_t *id)
{
    ...
    memcpy(id,(uint8_t*)ChipUniqueID,12);
    
    return 0;
}
static uint32_t Get_NetMAC(uint8_t *mac)
{
    uint8_t Tmp[36] = {0};
    Get_Chip_ID(Tmp);
    Tmp[0] = 0xb8;
...
static void low_level_init(struct netif *netif)
{
  ...
  EthHandle.Init.PhyAddress = 0;//DP83848_PHY_ADDRESS;
...
void ethernetif_input( void const * argument )
{
  ...
  for( ;; )
  {
    ...
    {
      while(((EthHandle.RxDesc->Status & ETH_DMARXDESC_OWN) == (uint32_t)RESET))
      {
